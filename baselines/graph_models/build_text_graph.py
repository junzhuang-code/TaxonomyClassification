...
# Select the dataset
data_id = '1'
data_name = f"llm_survey{data_id}"

# Define the path
data_path = '../../data/text_corpus/'
input_path = data_path+"clean_corpus"
output_path = data_path+"text_graph"


def get_window(content_lst, window_size):
    """
    找出窗口
    :param content_lst, window_size;
...
class BuildGraph:
    def __init__(self, dataset):
        ...
        self.text_name = f"{self.input_path}/{self.dataset}.txt"
        self.graph_name = f"{self.output_path}/{self.dataset}.txt"
    ...
    def get_pmi_edge(self):
        pmi_edge_lst, self.pmi_time = get_pmi_edge(self.text_name, window_size=20, threshold=0.0)
    ...
    def get_tfidf_vec(self):
        ...
        tfidf_vec = text_tfidf.fit_transform(open(self.text_name, "r"))
    ...
    def save(self):
        ...
        nx.write_weighted_edgelist(self.g, self.graph_name)
        print("\n")


if __name__ == '__main__':
    BuildGraph(data_name)
